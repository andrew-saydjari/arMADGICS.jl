# E7 variant of run_M123_fixture.jl (E6 driver): identical pipeline path, but
# the optional 4th arg is a PER-FIBER starLines prior path template
# (e.g. .../APOGEE_stellar_kry_50_subpix_f); "<template><NNN>.h5" is loaded as
# V_starlines for each fiber, replacing the production
# `V_starlines = V_starlines_refLSF` hack. Without the 4th arg the hack is
# kept (production behavior), so before/after arms use identical code.
#
# Usage:
#   julia --project=<env> scripts/validation/run_M123_fixture_starlines.jl \
#       <src_dir> <fixture_dir> <out_h5> [starLines_perfiber_template]
#
# Priors (read-only): the production 2025_07_31 set from pipeline.jl (starCont
# kept at the production rough prior in both arms).
...
starLines_template = length(ARGS) >= 4 ? ARGS[4] : nothing
println("starLines per-fiber template: ", something(starLines_template, "NONE (refLSF hack, production)"))
...
# V_starlines is per-fiber when a template is given (E7); otherwise the
# production refLSF hack. prior_vec is assembled inside the fiber loop.
function make_prior_vec(adjfiberindx)
    V_starlines = if starLines_template === nothing
        V_starlines_refLSF # hack (matches production)
    else
        h5read(starLines_template * lpad(adjfiberindx, 3, "0") * ".h5", "Vmat")
    end
...
h5open(out_h5, "w") do fh
    ...
    fh["errmsg"] = errmsg
    fh["RV_minchi2_final"] = rv_minchi2
    attrs(fh)["src_dir"] = src_dir
    attrs(fh)["fixture_dir"] = fixture_dir
    attrs(fh)["starCont_prior"] = prior_dict["starCont"]
    attrs(fh)["starLines_perfiber_template"] = something(starLines_template, "NONE_refLSF_hack")
end
