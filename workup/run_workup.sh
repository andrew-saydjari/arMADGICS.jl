...
#
# MPI launch logic:
#   * inside a multi-node Slurm allocation → srun --mpi=pmix --ntasks(-per-node)
#   * single node (ccalin051, or a 1-node allocation) → mpiexec -np N
#   * N = WORKUP_RANKS: default "auto" → WorkupSerial.auto_ranks (data +
#     per-node resources; WORKUP_MEM_FRACTION=0.90 is the only tunable);
#     an explicit integer is honored as the total.
#
...
usage() {
    ...
    echo "       WORKUP_MEM_FRACTION=F (default 0.90)" >&2
    exit 2
}
...
if [ "${WORKUP_RANKS:-auto}" = "auto" ]; then
    read -r RANKS RANKS_PER_NODE < <(julia --project="$WORKUP_DIR" -e "
        include(joinpath(\"$WORKUP_DIR\", \"WorkupSerial.jl\"))
        println(join(WorkupSerial.auto_ranks(\"$RAWDIR\"; fibers=$FIBERS,
            frac=${WORKUP_MEM_FRACTION:-0.90}), \" \"))")
    echo "[run_workup] ranks: $RANKS ($RANKS_PER_NODE/node) [auto: fibers $FIBERS, ${NNODES} node(s), frac ${WORKUP_MEM_FRACTION:-0.90}]"
else
    RANKS=$WORKUP_RANKS; RANKS_PER_NODE=$(( (RANKS + NNODES - 1) / NNODES ))
...
if [ -n "${SLURM_JOB_ID:-}" ] && [ "${SLURM_JOB_NUM_NODES:-1}" -gt 1 ]; then
    ...
    # --use-hwthread-cpus: slots = hardware threads, matching Sys.CPU_THREADS
    exec mpiexec --use-hwthread-cpus -np "$RANKS" julia --project="$MPI_ENV" "${WRITER[@]}"
fi
